...
- **The Homebridge log showed `[eWeLink]` instead of `[eWeLink]`**: Homebridge derives a plugin's log prefix from `name` in its platform config, falling back to the plugin alias when that key is absent. `config.schema.json` declared a `name` property but never listed it in `layout`, so the settings form never rendered the field and never wrote its default into `config.json`. `name` is now the first control in the form and defaults to `eWeLink`.
...
- **Removed the unused `node-persist` dependency** (and its `@types` package). Nothing in the plugin imported it; token persistence uses its own file-based storage.
...
- **HomeKit naming (Homebridge 2.0)**: Sanitize device, RF sub-device, and RF button names before assigning them to HomeKit `Name` / `ConfiguredName` characteristics. Disallowed characters (e.g. `/`, `_`, emojis) are replaced with spaces, silencing the `HAP-NodeJS WARNING: ... has an invalid 'ConfiguredName' characteristic` log spam introduced by HAP-NodeJS' stricter validation. Cached accessories with previously-stored invalid names are also rewritten on load.
...
- ESLint 10 lint fixes

## [1.0.30] - 2026-03-30

### Changed

- **Dependencies**: Updated all dependencies to latest versions including `@homebridge/plugin-ui-utils` ^2.2.3, `axios` ^1.14.0, `fakegato-history` ^0.6.7, `node-persist` ^4.0.4, `ws` ^8.20.0, `eslint` ^10.1.0, `typescript` ^6.0.2, `vitest` ^4.1.2, and other dev dependencies.
...
- **Device support**: Sonoff Zigbee MINI DUO-L / MINI-ZB2GS-L 2-channel switch (UIID 7029)
- **Device support**: Sonoff MINI-ZBRBS Zigbee curtain motor (UIID 7034)
- **Device support**: NSPanel Pro 120 (UIID 228) — temperature sensor + Zigbee coordinator
- **Device support**: SONOFF SNZB-02LD Zigbee temperature sensor (UIID 7033)

## [1.0.27] - 2026-03-07

### Added

- **Device support**: SONOFF S61STPF smart plug (UIID 276) — single-channel outlet with full power monitoring ([#744](https://github.com/homebridge-plugins/homebridge-ewelink/issues/744))

## [1.0.26] - 2026-03-06

### Fixed

- **Nonce length**: Fixed regression introduced in v1.0.25 — `randomBytes(4).toString('hex')` produces the 8-character nonce required by the eWeLink API (v1.0.25 used `randomBytes(8)` which produced 16 chars, causing a 400 error on login)
...
### Fixed
- **Bridge Devices Visible in HomeKit**: Sonoff RF Bridge and ZB Bridge devices no longer appear as accessories in HomeKit
  - Bridge devices are now intercepted before accessory registration
  - Previously cached bridge accessories are automatically removed on restart
  - RF event routing to sub-devices (buttons, sensors) continues to work as before

## [1.0.20] - 2026-01-23

### Fixed
- **RF Button Names in HomeKit**: Fixed button names not displaying correctly in HomeKit
  - Added `ConfiguredName` characteristic for proper HomeKit display
  - Each button now shows its individual name instead of the accessory name
