...
export class HeaterAccessory extends BaseAccessory {
  ...
  constructor(
    ...
    // Check cache type
    if (accessory.context.cacheType !== 'heater') {
      accessory.context.cacheType = 'heater';
      accessory.context.cacheTarget = 20;
    }
  ...
  private async setActive(value: CharacteristicValue): Promise<void> {
    await this.handleSet(value as number, 'Active', async (active) => {
      ...
      if ((active === 0 && this.cacheHeat === 'on') || (active === 1 && newHeat === 'on')) {
        const params = SwitchHelper.buildSwitchParams(this.deviceParams, this.channelIndex, newHeat === 'on');
