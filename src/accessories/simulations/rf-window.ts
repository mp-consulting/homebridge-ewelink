 ...
 * Uses RF bridge buttons to control a window with position tracking
 * Buttons: open, stop, close (stored in accessory.context.buttons)
 */
export class RFWindowAccessory extends BaseAccessory {
  ...
  constructor(
    ...
    // Remove old services (switch services, conflicting WindowCovering/Door services)
    this.removeServiceIfExists(this.Service.WindowCovering);
    this.removeServiceIfExists(this.Service.Door);
    ...
    // Set up Window service
    this.service = this.getOrAddService(this.Service.Window);
    ...
    this.logInfo(`Initialized as RF window (operation time: ${this.operationTimeUp / 10}s up, ${this.operationTimeDown / 10}s down)`);
  }
  ...
  /**
   * Set target position - controls window movement via RF transmit commands
   */
  private async setTargetPosition(value: CharacteristicValue): Promise<void> {
    ...
    try {
      ...
      this.logError('Failed to set window position:', err);
