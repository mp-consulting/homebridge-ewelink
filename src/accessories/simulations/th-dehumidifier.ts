 ...
 */
export class THDehumidifierAccessory extends BaseAccessory {
  ...
  constructor(
    ...
    // Set dehumidifier as primary service
    ...
    // Configure target state (dehumidifier only)
    ...
    this.service.getCharacteristic(this.Characteristic.RelativeHumidityDehumidifierThreshold)
    ...
    this.cacheHumid = this.cacheState === 'on' &&
      this.service.getCharacteristic(this.Characteristic.CurrentHumidifierDehumidifierState).value === 3
      ? 'on'
      : 'off';
  ...
  private async setActive(value: CharacteristicValue): Promise<void> {
    await this.handleSet(value as number, 'Active', async (active) => {
      ...
      if (active === 0) {
        ...
        // Humidity above target, turn on dehumidifier
        params.mainSwitch = 'on';
        params.switch = 'on';
        newState = 'on';
        newHumid = 'on';
      } else {
        // Humidity at or below target, turn off dehumidifier
        params.mainSwitch = 'off';
        params.switch = 'off';
        newState = 'on';
        newHumid = 'off';
      ...
      if (newState !== this.cacheState) {
        ...
        this.logDebug(`Dehumidifier state: ${this.cacheState}`);
      }

      if (newHumid !== this.cacheHumid) {
        this.cacheHumid = newHumid;
        this.logDebug(`Dehumidifying: ${this.cacheHumid}`);
      }
  ...
  private async setTargetHumidity(value: CharacteristicValue): Promise<void> {
    ...
    // Update dehumidifying state based on new target
    await this.updateDehumidifyingState();
  }
  ...
  /**
   * Update dehumidifying state based on current humidity vs target
   */
  private async updateDehumidifyingState(): Promise<void> {
    try {
      if (this.cacheState === 'off') {
        return;
      }
      ...
      this.logError('Failed to update dehumidifying state', err);
    }
  }
  ...
  updateState(params: DeviceParams): void {
    ...
    if (params.currentHumidity !== undefined && params.currentHumidity !== 'unavailable') {
      ...
      if (newHumi !== this.cacheHumi) {
        ...
        // Update dehumidifying state when humidity changes
        this.updateDehumidifyingState();
      }
    }
  }
}
